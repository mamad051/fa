local function check_member_superrem2(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  chat_del_user(get_receiver(msg), 'user#id'..232111594, ok_cb, false)
...
function run(msg, matches)
	if matches[1]:lower() == 'setexpire' or matches[1] == 'تنظیم انقضا' then
	...
	if matches[1]:lower() == 'expire' or matches[1] == 'انقضا' then
		local expiretime = redis:hget ('expiretime', get_receiver(msg))
